<?php
    ...
    class AbstractModel
    {
        ...
        public function checkProperties($except = [])
        {
            foreach(static::$tableSchema as $property => $type){
        ...
        private function create()
        {
            ...
            $stmt = PDODatabaseHandler::getInstance()->prepare($sql);
            $this->prepareValues($stmt);
            if($stmt->execute()) {
                $this->{static::$primaryKey} = PDODatabaseHandler::getInstance()->lastInsertId();
                return true;
            }
            return false;
        }
        ...
        public static function getOneBy($columns, $options = array())
        {
            $whereClauseColumns = array_keys($columns);
            $whereClauseValues = array_values($columns);
            $whereClause = [];
